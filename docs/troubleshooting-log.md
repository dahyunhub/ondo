...
메모리 카운터가 더 싸지만 **Cloud Run 에서는 보증이 되지 않는다.** 인스턴스가 늘면 같은 공격자의 요청이 서로 다른 카운터에 흩어지고, 재배포·콜드스타트마다 초기화된다. MySQL 은 이미 있으므로 새 인프라 없이 `auth_attempt`(V10) 한 장으로 해결했다. `(scope, attempt_key)` 복합 PK 의 고정 창 방식이고, 창이 지나면 같은 행을 재사용해 정상 사용자 쪽으로는 행이 늘지 않는다.

| 스코프 | 키 | 기본 정책 |
|---|---|---|
| `LOGIN` | 정규화된 이메일 | 15분 10회 → 15분 잠금 |
| `PASSWORD_CONFIRM` | teacherId | 15분 5회 → 15분 잠금 |
| `RESET_REQUEST` | 정규화된 이메일 | 1시간 5회 → 1시간 발송 중단 |

지연(sleep)은 쓰지 않았다 — 요청 스레드를 붙잡는 문제가 이미 `OpenAiClient` 이연 항목으로 지적돼 있다.

### 이 기능이 스스로를 무너뜨릴 뻔한 지점 셋

1. **실패 횟수가 롤백에 휩쓸린다.** 로그인 실패는 예외로 끝나 바깥 트랜잭션이 롤백된다. 같은 트랜잭션에서 세면 **실패 기록까지 함께 사라져 가드가 통째로 무력**해진다. `AuthThrottleService` 를 별도 빈 + `REQUIRES_NEW` 로 뒀다(자기호출은 프록시를 타지 않아 전파가 먹지 않는다 — 아키텍처 리뷰 #5 와 같은 함정).
2. **429 가 계정 존재를 알려준다.** 제한을 계정 조회보다 **먼저** 걸어 미가입 이메일도 똑같이 세고 똑같이 429 를 준다. 재설정 요청은 아예 429 를 쓰지 않는다 — 거기서 429 를 주면 그 자체가 "가입된 이메일"이라는 신호가 되어, 그 API 가 지키려던 계정 열거 방지가 무너진다. 응답은 그대로 두고 발송만 조용히 멈춘다.
3. **대소문자로 우회된다.** MySQL 기본 콜레이션이 대소문자를 구분하지 않아 `A@b.com` 으로도 같은 계정에 로그인된다. 키를 소문자로 정규화하지 않으면 대소문자만 바꿔 카운터를 늘릴 수 있다.

### 테스트 — 두 번 깨지고 배운 것

**① 정리 코드가 락을 쥐고 있었다.** `@BeforeEach` 의 `DELETE FROM auth_attempt` 를 테스트 트랜잭션 안에서 돌렸더니, 그 트랜잭션이 테이블에 락을 쥔 채 본문으로 들어가고 본문의 `REQUIRES_NEW` 인서트가 그 락을 기다렸다. `innodb_lock_wait_timeout`(50초)마다 500 이 나면서 **16건 실패·전체 18분**. 정리를 별도 트랜잭션에서 커밋하도록 바꿔 26초로 복귀.

**② 트랜잭션으로 감싼 테스트는 이 가드를 볼 수 없다.** 가드의 구조가 "실패는 `REQUIRES_NEW` 로 커밋하고 다음 요청이 그것을 읽는다"라 **트랜잭션 경계를 넘는 가시성 자체가 검증 대상**이다. 테스트를 한 트랜잭션으로 감싸면 REPEATABLE READ 스냅샷이 나중에 커밋된 카운터를 보지 못해, 실서비스에서는 잘 도는 가드가 테스트에서만 안 걸린다. 해당 클래스는 `@Transactional` 을 쓰지 않고 만든 데이터를 직접 지운다. 실서비스는 요청마다 트랜잭션이 따로라 해당 없음.

신규 7건 포함 **219개 통과**.

### 검증

- 라이브(`ondo-api-00019-nqc`)에서 **미가입 이메일**로 10회 실패 후 11회째부터 429 `TOO_MANY_ATTEMPTS` 확인 — 존재하지 않는 계정도 동일하게 막히므로 429 가 계정 존재 신호가 되지 않는다.
- 정상 로그인 200 은 그대로. Flyway V10 적용 확인.

### 알려진 한계

- **정당한 사용자를 잠글 수 있다.** 교사 이메일을 아는 사람이 일부러 실패시켜 15분간 로그인을 막을 수 있다. 잠금이 짧고, 열린 오라클을 두는 것보다 낫다고 판단해 감수했다.
- 정리 잡은 `@Scheduled` 라 min-instances=0 에서는 인스턴스가 자면 그날 건너뛴다. 행이 아주 작아 best-effort 로 둔다.
