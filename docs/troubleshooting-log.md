...
<a id="gzip"></a>
## 2026-08-09 · 정적 자산·API 응답이 무압축으로 나가던 문제 (사후 기록)

> 이 항목은 2026-09-13에 뒤늦게 적었다. 작업 당시엔 이 로그가 6월에서 멈춰 있었고, 근거는 커밋
> `perf(web): nginx 응답 gzip 압축 활성화`(2026-08-09)와 `bench/results/before.md`·`after.md`(8/5·8/6 측정)다.

**맥락:** `bench/` 하네스를 처음 만들어 1학기치 데이터로 재 보니, 프론트 번들과 API JSON 이 전부 **압축 없이** 나가고 있었다. nginx 기본 설정이 gzip 을 켜지 않는다.

**수정:** `frontend/nginx.conf` 에 gzip 활성화(텍스트 계열 MIME 대상).

### 측정 (bench, 반 1 · 아이 23명 · 메모 3,089건 · 사진 21장 5.7MB)

| 지표 | before | after |
|---|---:|---:|
| 초기 로딩 전송량(index 가 참조하는 JS+CSS) | 282.9KB | **79.6KB** (−71.9%) |
| JS 번들 | 210.8KB | **67.7KB** (−67.9%) |
| CSS | 72.2KB | **11.9KB** (−83.5%) |
| 일지 목록 응답 | 57.7KB | **2.4KB** (−95.8%) |
| 타임라인 응답 | 67.7KB | **4.1KB** (−93.9%) |
| 아이 명단 p95 | 63.1ms | **20.2ms** (−68.0%) |

- **DB→앱 전송량과 읽은 행 수는 전 항목 동일** — 쿼리는 손대지 않고 전송 계층만 바뀌었다는 증거.
- **사진 응답(323KB)만 변화 없음** — JPEG 는 이미 압축돼 있어 gzip 이 먹지 않는다. 이게 정상 동작의 증거이기도 하다.
- 이때 아이 명단의 `db_kb_per_req` 가 5,678KB 로 찍혀 있었다. 응답 2.9KB 를 만들자고 5.6MB 를 읽는다는 뜻이었는데, **한 달 뒤(2026-09-09 항목)에야 원인을 찾았다.** 하네스가 문제를 드러냈지만 읽는 사람이 없으면 소용없다.
