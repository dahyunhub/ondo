...
const props = defineProps({ children: { type: Array, required: true } })

const router = useRouter()
function openMemo(c) { router.push({ name: 'memo', query: { childId: c.id } }) }

// 카드가 길어지지 않게 한 페이지 3명씩 나눈다(4명 이상부터 페이지가 생긴다).
const PAGE_SIZE = 3
const page = ref(1)
const totalPages = computed(() => Math.max(1, Math.ceil(props.children.length / PAGE_SIZE)))
const pagedChildren = computed(() => props.children.slice((page.value - 1) * PAGE_SIZE, page.value * PAGE_SIZE))
// 목록이 줄어(다른 곳에서 갱신) 현재 페이지가 사라지면 마지막 페이지로 보정.
...
<template>
  <div class="jr-card meet">
    ...
    <div class="meet-list">
      <div v-for="c in pagedChildren" :key="c.id" class="meet-row">
    ...
    <Pagination v-if="totalPages > 1" :page="page" :total-pages="totalPages" @update:page="(p) => (page = p)" />
  </div>
</template>
