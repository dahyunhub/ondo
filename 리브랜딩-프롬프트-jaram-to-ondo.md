...
> 아래 블록 전체를 코딩 에이전트(Claude Code 등)에게 그대로 붙여넣으면 됩니다.
> 작업 전 반드시 `git`이 깨끗한 상태(또는 새 브랜치 `rename/ondo`)인지 확인하세요.
...
- **이 리브랜딩 결정과 변경 내역을 `_bmad-output/리뷰-트러블슈팅-로그.md`에 한 항목으로 추가**하라 (날짜 2026-06-19, 사유: 영유아 교육 업계 동명 브랜드 회피 및 검색 차별성 확보).
